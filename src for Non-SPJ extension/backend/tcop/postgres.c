...
exec_simple_query(const char* query_string)
{
	...
	if (strstr(query_string, "switch mu to ") != NULL)
	{
		int flag = sscanf(query_string, "switch mu to %lf;", &mu);
	}

	if (strstr(query_string, "switch sigma to ") != NULL)
	{
		int flag = sscanf(query_string, "switch sigma to %lf;", &sigma);
	}
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		else
		{
			...
			if (snapshot_set)
				PopActiveSnapshot();
			Query* q = (Query*)lfirst(list_head(querytree_list));
			MemoryContext mc = MemoryContextSwitchTo(MessageContext);
			Query* q2 = copyObjectImpl(q);
			MemoryContextSwitchTo(mc);
			doQSparse(query_string, commandTag, q2, completionTag, dest);
		}
